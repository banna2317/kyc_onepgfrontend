...
async function getrecords(page = 1) {
    try {
        ...
        const response = await axios.post(`/pan-lite-records?page=${page}`);
...
async function handleViewRecord(id) {
    ...
    try {
        const response = await axios.post("/pan-lite-recordID", { id: id });
...
const handleFilter = async (filters: {
    ...
    try {
        const response = await axios.post("/filterdata", {
            dataType: "pancard_lite_verification",
...
<template>
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-0">
            <div>
                <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight">Pancard Lite Verification</h1>
