...
async function handleVerify() {
    ...
    try {
        const response = await axios.post("/aadhaar-validation", {
...
async function getrecords(page = 1) {
    try {
        const response = await axios.post(`/aadhaar-validation-records?page=${page}`);
...
async function handleViewRecord(id) {
    try {
        const response = await axios.post("/aadhaar-validation-recordID", { id: id });
...
<template>
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-0">
            <div>
                <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight">Aadhaar Validation</h1>

                <div class="flex flex-col sm:flex-row sm:items-center gap-1 mt-2">
                    <p class="text-sm sm:text-base text-muted-foreground">
                        Verify Aadhaar Validation. <Button variant="link"
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    <TableRow v-for="record in allRecords" :key="record.id" class="hover:bg-muted/50">
                                        ...
                                        <TableCell class="font-mono">{{ record.state ?? "N/A" }}</TableCell>
                                        <TableCell class="font-mono">{{ record.gender ?? "N/A" }}</TableCell>
            ...
            <DialogContent class="sm:max-w-md">
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <Input id="aadhaar " placeholder="Enter Aadhaar  Number" v-model="aadhaar_number" />
                    </div>
            ...
            <DialogContent class="sm:max-w-lg">
                ...
                <div v-if="selectedRecord" class="space-y-4">
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
                            <span class="text-muted-foreground font-medium">Age Range</span>
                            <span class="font-mono">
                                {{ selectedRecord.age_range ?? "N/A" }}
                            </span>
                        </div>
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
                            <span class="text-muted-foreground font-medium">masked Mobile Number</span>
                            <span class="font-mono">
                                {{ selectedRecord.masked_mobile_number ?? "N/A" }}
                            </span>
                        </div>
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
                            <span class="text-muted-foreground font-medium">Status Code</span>
                            <span class="font-mono">
                                {{ selectedRecord.aadhaar_status_code ?? "N/A" }}
                            </span>
                        </div>
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
