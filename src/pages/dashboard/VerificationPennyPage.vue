...
async function getrecords(page = 1) {
    try {
        const response = await axios.post(`/verify-bank-account-penny-records?page=${page}`, {
        ...
        allRecords.value = res.data.map((record: any) => ({
            id: record.id,
            verifiedAt: formatDate(record.created_at),
            verificationId: record.verification_id,
            accountNumber: record.account_number,
            ifsc: record.ifsc_code,
            nameAtBank: record.full_name,
            accountStatus:
                record.account_exists === true ? "Active" :
                    record.account_exists === false ? "Inactive" : "Dormant"
        }));
...
async function handleVerify() {
    ...
    try {
        const response = await axios.post("/verify-bank-account-penny", {
            account_number: formData.value.accountNumber,
            ifsc_code: formData.value.ifsc,
        });
        getrecords();
...
// 🔄 Reset form
function resetForm() {
    formData.value = { accountNumber: "", ifsc: "", };
    verificationResult.value = null;
    isDialogOpen.value = false;
}
...
async function handleViewRecord(id) {
    ...
    try {
        const response = await axios.post("/verify-bank-account-penny-recordID", { id: id });
...
const handleFilter = async (filters: {
    ...
    try {
        ...
        allRecords.value = res.data.map((record: any) => ({
            ...
            accountStatus:
                record.account_exists === true ? "Active"
                    : record.account_exists === false ? "Inactive"
                        : "Dormant",
        }))
        function formatDate(isoString: string) {
            const date = new Date(isoString);
...
<template>
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-0">
            <div>
                <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight">Bank Account Verification-penny
                    Drop</h1>

                <div class="flex flex-col sm:flex-row sm:items-center gap-1 mt-2">
                    <p class="text-sm sm:text-base text-muted-foreground">
                        Verify bank accounts   <Button variant="link" class="p-0 h-auto text-blue-600 hover:text-blue-800">
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    <TableRow v-for="record in filteredRecords" :key="record.id"
                                        ...
                                        <TableCell class="font-mono">{{ record.accountNumber }}</TableCell>
                                        <TableCell class="font-mono">{{ record.ifsc }}</TableCell>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <Input id="accountNumber" placeholder="Account Number" v-model="formData.accountNumber" />
                        </div>

                        <div class="space-y-2">
                            <Label for="ifsc">IFSC</Label>
                            <Input id="ifsc" placeholder="IFSC" v-model="formData.ifsc" />
                        </div>
                    </div>
                    ...
                    <div v-if="verificationResult" class="space-y-4 p-4 bg-muted/30 rounded-lg border">
                        ...
                        <div v-if="verificationResult.success" class="space-y-2 text-sm">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium">{{ verificationResult.bankName }}</span>
                            </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        ...
                        <Button v-if="!verificationResult" @click="handleVerify"
                ...
                <div v-if="selectedRecord" class="space-y-4">
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <div class="flex justify-between items-center py-2">
                            ...
                            <span class="font-semibold text-green-600">₹{{ selectedRecord.fees }}</span>
                        </div>
                    </div>
