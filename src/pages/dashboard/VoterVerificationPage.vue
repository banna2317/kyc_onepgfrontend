...
async function handleVerify() {
    ...
    if (!voter_id.value.trim()) {
        toast({
            title: "Error",
            description: "Please enter voter id Number",
            variant: "destructive"
        });
        return;
    }
    try {
        const response = await axios.post("/voter-verification", {
...
async function getrecords(page = 1) {
    try {
        ...
        const response = await axios.post(`/voter-verification-records?page=${page}`, {
...
async function handleViewRecord(id) {
    try {
        ...
        const response = await axios.post("/voter-verification-recordID", { id: id });
...
<template>
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-0">
            <div>
                <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight">Voter ID Verification</h1>
        ...
        <div class="flex items-center gap-4">
            <FilterDropdown :search-options="[
                { label: 'Voter ID Number', value: 'voter_id' },
            ...
            <DialogContent class="sm:max-w-md">
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <Input id="voter" placeholder="Enter Voter ID Number" v-model="voter_id" />
                    </div>

                    <div class="flex justify-end gap-3 pt-4">
                        <Button @click="handleVerify" :disabled="!voter_id" class="bg-purple-600 hover:bg-purple-700">
            ...
            <DialogContent class="sm:max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <div v-if="selectedRecord" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <!-- Voter ID -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
                            <span class="text-muted-foreground font-medium">EPIC / Voter ID</span>
                            <span class="font-mono">
                                {{ selectedRecord.voter_id ?? "N/A" }}
                            </span>
                        </div>
                        ...
                        <!-- Age -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
                            <span class="text-muted-foreground font-medium">Age</span>
                            <span class="font-mono">
                        ...
                        <!-- Area -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
                            <span class="text-muted-foreground font-medium">Area</span>
                            <span class="font-mono">
                        ...
                        <!-- Polling Station -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
                            <span class="text-muted-foreground font-medium">Polling Station</span>
                            <span class="font-mono break-words">
                                {{ selectedRecord.polling_station ?? "N/A" }}
                            </span>
                        </div>

                        <!-- Parliamentary Constituency -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
                            <span class="text-muted-foreground font-medium">Parliamentary Constituency</span>
                            <span class="font-mono">
                                {{ selectedRecord.parliamentary_constituency ?? "N/A" }}
                            </span>
                        </div>

                        <!-- Assembly Constituency -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
                            <span class="text-muted-foreground font-medium">Assembly Constituency</span>
                            <span class="font-mono">
                                {{ selectedRecord.assembly_constituency ?? "N/A" }}
                            </span>
                        </div>
