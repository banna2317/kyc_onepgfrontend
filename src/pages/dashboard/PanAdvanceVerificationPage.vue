...
async function handleVerify() {
    ...
    try {
        const response = await axios.post("/pan-advance", {
        ...
        toast({
            title: "Success",
            description: response.data.message || "Pancard verified successfully.",
        });
...
async function getrecords(page = 1) {
    try {
        ...
        const response = await axios.post(`/pan-advance-records?page=${page}`,
            {
...
async function handleViewRecord(id) {
    try {
        const response = await axios.post("/pan-advance-recordID", { id: id });
...
const handleFilter = async (filters: {
    ...
    try {
        const response = await axios.post("/filterdata", {
            dataType: "panadvance_verification",
...
<template>
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-0">
            <div>
                <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight">Pancard Advance Verification</h1>

                <div class="flex flex-col sm:flex-row sm:items-center gap-1 mt-2">
                    <p class="text-sm sm:text-base text-muted-foreground">
                        Verify Pancard Advance. <Button variant="link"
