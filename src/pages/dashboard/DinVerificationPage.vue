...
async function handleVerify() {
    ...
    try {
        const response = await axios.post("/din-verification", {

            din_number: din_number.value,
...
async function getrecords(page = 1) {
    try {
        ...
        const response = await axios.post(`/din-verification-records?page=${page}`, {
...
async function handleViewRecord(id) {
    try {
        ...
        const response = await axios.post("/din-verification-recordID", { id: id });
    ...
    <div class="space-y-6">
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow class="border-b">
                                        ...
                                        <TableHead>DIN Number</TableHead>
                                ...
                                <TableBody>
                                    <TableRow v-for="record in allRecords" :key="record.id" class="hover:bg-muted/50">
                                        ...
                                        <TableCell class="font-mono">{{ record.din_number }}</TableCell>
        ...
        <Dialog :open="isDialogOpen" @update:open="val => isDialogOpen = val">
            <DialogContent class="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle>Verify DIN </DialogTitle>
                </DialogHeader>
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center gap-2">
                            <Label for="din">Director Identification Number </Label>

                        </div>
                        <Input id="din" placeholder="Enter Director Identification Number" v-model="din_number" />
                    </div>

                    <div class="flex justify-end gap-3 pt-4">
                        <Button @click="handleVerify" :disabled="!din_number" class="bg-purple-600 hover:bg-purple-700">
            ...
            <DialogContent class="sm:max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <div v-if="selectedRecord" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
                            <span class="text-muted-foreground font-medium">Director Identification Number</span>
                            <span class="font-mono">
                                {{ selectedRecord.din_number ?? "N/A" }}
                            </span>
                        </div>

                        <!-- Father Name -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50"
                            v-if="selectedRecord.father_name">
                        ...
                        <!-- Full Name -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50"
                            v-if="selectedRecord.full_name">
                            <span class="text-muted-foreground font-medium">Full Name</span>
                            <span class="font-mono">{{ selectedRecord.full_name }}</span>
                        </div>

                        <!-- DOB -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50"
                            v-if="selectedRecord.dob">
                            <span class="text-muted-foreground font-medium">Date of Birth</span>
                            <span class="font-mono">{{ selectedRecord.dob?.substring(0, 10) }}</span>
                        </div>

                        <!-- Nationality -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50"
                            v-if="selectedRecord.nationality">
                            <span class="text-muted-foreground font-medium">Nationality</span>
                            <span class="font-mono">{{ selectedRecord.nationality }}</span>
                        </div>

                        <!-- Present Address -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50"
                            v-if="selectedRecord.present_address">
                            <span class="text-muted-foreground font-medium">Present Address</span>
                            <span class="font-mono">{{ selectedRecord.present_address }}</span>
                        </div>

                        <!-- Permanent Address -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50"
                            v-if="selectedRecord.permanent_address">
                            <span class="text-muted-foreground font-medium">Permanent Address</span>
                            <span class="font-mono">{{ selectedRecord.permanent_address }}</span>
                        </div>

                        <!-- Email -->
                        <div class="flex justify-between items-center py-2 border-b border-border/50"
                            v-if="selectedRecord.email">
                            <span class="text-muted-foreground font-medium">Email</span>
                            <span class="font-mono">{{ selectedRecord.email }}</span>
                        </div>
                        ...
                        <div class="flex justify-between items-center py-2 border-b border-border/50"
                            ...
                            <span class="font-mono">{{ selectedRecord.pan_number }}</span>
                        </div>

                        <div class="flex justify-between items-center py-2 border-b border-border/50"
                            v-if="selectedRecord.status">
                            <span class="text-muted-foreground font-medium">Status</span>
                            <span class="font-mono capitalize">{{ selectedRecord.status }}</span>
                        </div>
                        <div class="flex justify-between items-center py-2 border-b border-border/50">
                            <span class="text-muted-foreground font-medium">Fees</span>
                            <span class="font-mono capitalize">{{ selectedRecord.fees }}</span>
                        </div>
