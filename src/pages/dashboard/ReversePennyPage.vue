...
async function getrecords(page = 1) {
  try {
    const response = await axios.post(`/verify-bank-account-penny-less-records?page=${page}`, {
    ...
    allRecords.value = res.data.map((record: any) => ({
      ...
      accountNumber: record.account_number,       // ✅ maps to template
      ifsc: record.ifsc_code || record.ifsc,      // ✅ maps to template

      nameAtBank: record.full_name,            // ✅ maps to template
...
async function handleVerify() {
  ...
  try {
    const response = await axios.post("/verify-bank-account-penny-less", {
...
async function handleViewRecord(id) {
  ...
  try {
    const response = await axios.post("/verify-bank-account-penny-less-recordID", { id: id });
    if (response.data.status === true) {
...
const handleFilter = async (filters: {
  ...
  try {
    const response = await axios.post("/filterdata", {
      dataType: "bankbank_account_penny_less",
      ...
      <div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-1 mt-2">
          <p class="text-sm sm:text-base text-muted-foreground">
            Verify bank accounts  <Button variant="link"
